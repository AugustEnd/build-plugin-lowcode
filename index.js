...
function formatComponentSchema(schema) {
	...
	if (props && Array.isArray(props)) {
		props.forEach((prop) => {
			// console.log(1, prop, prop.defaultValue);
...
function getUsedComponentMetas(
	...
	let metaPaths = glob.sync(
		path.resolve(
			rootDir,
			`${lowcodeDir}/**/${metaFilename}.@(js|ts|jsx|tsx)`
		)
	);
...
async function build(options, pluginOptions, execCompile) {
	...
	if (type === undefined || type === "only-generator") {
		!noParse &&
...
async function initLowCodeSchema(
	...
	if (lowcodeDirExists) {
		const lowcodeDirs = await fse.readdir(
			path.resolve(rootDir, lowcodeDir)
		);
		...
		const newComponentDir = componentsDirs.filter(
			(dir) => !lowcodeDirMap[dir]
		);
	...
	const filteredComponents =
		!components || !components.length
			? result
			: result.filter(
					(item) => item && components.includes(item.componentName)
			  );
	console.log(14, filteredComponents);
	filteredComponents.forEach((item) => {
		const componentNameFolder = camel2KebabComponentName(
			item.componentName
		);
		if (
			!fse.existsSync(
			...
			// console.log(20, schema.snippets);
...
async function bundleMetaV2(options, pluginOptions, execCompile, metaType) {
	...
	if (fullbackMeta) {
		fullbackMetaSuffix =
			fullbackMeta === "default" ? "" : `.${fullbackMeta}`;
...
async function bundleRenderView(options, pluginOptions, platform, execCompile) {
	...
	onGetWebpackConfig(`render-view-${platform}`, (config) => {
		...
		config.output.path(
			path.resolve(
				rootDir,
				`${buildTarget}/${lowcodeDir}/render/${platform}`
			)
		);
		config.externals({ ...COMMON_EXTERNALS, ...externals });
		if (baseLibrary === "rax") {
...
async function bundleComponentMeta(
	...
	usedDevComponents.forEach((component, idx) => {
		(function (comp, index) {
			...
			// console.log(17, componentsMetaPath);
			onGetWebpackConfig(taskName, (config) => {
				...
				config.output
					.library(componentMetaExportName)
					.libraryTarget("umd");
				config.output.path(
					path.resolve(rootDir, `${buildTarget}/${lowcodeDir}`)
				);
